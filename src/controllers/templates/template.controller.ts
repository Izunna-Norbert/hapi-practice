    ...
    return [
        {
            ...
            handler: async (request, h) => {
                //also return fields for each template, return select fields
                return await templateRepo.createQueryBuilder('templates')
                    .leftJoinAndSelect('templates.fields', 'fields')
                    .select(['templates.id', 'templates.title', 'templates.description', 'fields.id', 'fields.label', 'fields.type', 'fields.required', 'fields.placeholder'])
                    .getMany();
            }
        },
        {
            ...
            handler: async (request, h) => {
                ...
                const result = await templateRepo.createQueryBuilder('templates')
                    .leftJoinAndSelect('templates.fields', 'fields')
                    .select(['templates.id', 'templates.title', 'templates.description', 'fields.id', 'fields.label', 'fields.type', 'fields.required', 'fields.placeholder'])
